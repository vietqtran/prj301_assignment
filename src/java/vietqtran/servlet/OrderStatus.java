...
public class OrderStatus extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
	    throws ServletException, IOException {
	try {
	    long orderId = request.getParameter("orderId") != null ? Long.parseLong(request.getParameter("orderId")) : -1;
	    OrderDAO orderDao = new OrderDAO();
	    String status = request.getParameter("status").trim();
	    Order order = orderDao.get(orderId);
	    if (status.equals("waitingForDelivery") || status.equals("delivering")) {
		order.setStatus(status);
	    }
	    orderDao.update(order);
	    response.sendRedirect("shop-dashboard?tab=orders");
